...
- **Aim reticle** — a red, dashed, semi-transparent line projected from the center of the ship in the aim direction (mouse on desktop, right firing joystick on mobile). The line extends to the edge of the visible screen, or stops early when it intersects an asteroid. A red target circle (reticle) marks where shots will land: centered on the screen-edge endpoint when nothing is in the way, or snapped onto and sized to the intersected asteroid. Hidden during the scripted prologue.
